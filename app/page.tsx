...
export default function Home(){
 ...
 <section id="pillars" data-chapter className="pillars section">
  ...
  <div className="pillars-grid">{pillars.map(p=><article className="pillar" key={p.number} data-reveal><div className="pillar-index"><span>PILLAR {p.number}</span><span aria-hidden="true">{'\u2197\uFE0E'}</span></div><h3>{p.title}<br/>{p.subtitle}</h3><p className="pillar-purpose">{p.purpose}</p><p className="pillar-description">{withVeyagoLink(p.body)}</p><p className="pillar-scope">{p.scope}</p><a className="underlined" href={p.link}>{p.cta}<span aria-hidden="true">{'\u2197\uFE0E'}</span></a></article>)}</div>
 ...
 <section id="approach" data-chapter className="method-journey"><div className="method-sticky">
  ...
  <div className="phase-navigation" aria-label="Explore the four phases">{method.map(([n,title],i)=><button key={n} aria-pressed={phase===i} aria-controls="phase-details" onClick={()=>selectPhase(i)}><span className="phase-dot">{phase>i?'✓':n}</span><span>{title}</span><span className="phase-direction" aria-hidden="true">{'\u2197\uFE0E'}</span></button>)}</div>
  <div className="method-stage" data-phase={phase} data-paused={motionPaused}>
   ...
   <div id="phase-details" className="phase-details">{method.map(([n,title,headline,text,outcome],i)=><article key={n} data-active={phase===i}>
    <div className="phase-label"><span>PHASE {n}</span><span>4D / TDC DIGITAL ADVISORY</span></div><h3>{title}<span>.</span></h3><h4>{headline}</h4><p>{text}</p><div className="outcome"><small>What you get</small><span>{outcome} <b aria-hidden="true">{'\u2197\uFE0E'}</b></span></div>
   </article>)}</div>
  </div>
